...
function handleSearch(e) {
  ...
  renderFilteredTable([header, ...filteredData]);
}
...
function renderFilteredTable(filteredData) {
  ...
  document.getElementById('rowCount').textContent = `${body.length} fila${body.length !== 1 ? 's' : ''} (filtradas)`;
  document.getElementById('errorCount').textContent = `${invalidCellsCount} error${invalidCellsCount !== 1 ? 'es' : ''}`;
...
function setupValidation() {
  const requiredFields = ['apellido', 'nombre', 'numero_documento'];
  ...
  switch(fieldId) {
    ...
    case 'numero_documento':
      ...
      if ((tipoDoc === 'Pasaporte' || tipoDoc === 'OTROS') && 
        ...
        return 'Entre 5 y 100 caracteres';
      }
      break;
  }
  return '';
...
function displayFieldError(fieldId, error) {
  const field = document.getElementById(fieldId);
...
function renderTable() {
  ...
  document.getElementById('saveBtn').disabled = invalidCellsCount > 0;
  document.getElementById('rowCount').textContent = `${body.length} fila${body.length !== 1 ? 's' : ''}`;
  document.getElementById('errorCount').textContent = `${invalidCellsCount} error${invalidCellsCount !== 1 ? 'es' : ''}`;
...
function toggleSelectAll() {
  ...
  document.getElementById('selectAllBtn').textContent = 
    selectAllState ? 'Deseleccionar Todo' : 'Seleccionar Todo';
}
